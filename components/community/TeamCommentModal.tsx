...
const COPY = {
  title: { en: "NIIGHTMARE Fan Wall", lo: "ກຳແພງແຟນຄັບ NIIGHTMARE" },
  close: { en: "Close comments", lo: "ປິດຄອມເມັນ" },
  placeholder: { en: "Write a message for NIIGHTMARE...", lo: "ຂຽນຂໍ້ຄວາມເຊຍ NIIGHTMARE..." },
  send: { en: "Send", lo: "ສົ່ງ" },
  noComments: { en: "No team comments yet. Lead the chant.", lo: "ຍັງບໍ່ມີຄອມເມັນຂອງທີມ. ເລີ່ມເຊຍກ່ອນໄດ້ເລີຍ." },
  reviewQueued: {
  ...
  unavailable: { en: "Team community data is not ready yet.", lo: "ຂໍ້ມູນ Community ຂອງທີມຍັງບໍ່ພ້ອມ." },
};

function profileOf(comment: TeamComment) {
...
export default function TeamCommentModal({
  open,
  supportsCommunity,
  onClose,
}: {
  ...
  const loadComments = async () => {
    ...
    if (loadError) {
      ...
      setComments((data ?? []) as unknown as TeamComment[]);
    }
    setLoading(false);
  };
  ...
  const submitComment = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/community/team-comments", {
        method: "POST",
        headers: {
