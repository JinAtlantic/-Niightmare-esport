...
create table if not exists public.sponsors (
  ...
  url             text,
  logo            text,
  partner_group   text not null default 'official'
...
alter table public.sponsors add column if not exists partner_group text not null default 'official';
...
begin
  ...
  if jsonb_array_length(hydrated_rows) > 0 then
    execute format('insert into %1$s select * from jsonb_populate_recordset(null::%1$s, $1)', target)
      using hydrated_rows;
  end if;
end; $$;
